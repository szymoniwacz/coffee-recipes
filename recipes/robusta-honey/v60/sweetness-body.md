# V60 — Sweetness + body

## Quick Brew Card

- **Dose:** 15 g · **Water:** 190 g · **Grind:** Comandante 24 clicks · **Temp:** 92°C · **Brew time:** ~3:30
- **Pour schedule:** Bloom 30 g → +45 g (75 g) → +40 g (115 g) → +40 g (155 g) → +35 g (190 g)

### Taste Check
- Rest after brewing: not documented
- Best first sip: not documented
- Best taste range: not documented

## Short Version

1. Bloom 30 g (time TBD).
2. Pour 45 g, 40 g, 40 g, then 35 g (190 g total).
3. Target finish around 3:30.

## Status

- Status: tested
- Last updated: 2026-06-03

## Coffee

- Name: Xin Chao Hanoi (100% Robusta, speciality)
- Bean: Robusta
- Origin: Vietnam
- Process: Honey
- Roast level: Medium
- Roaster: 1996 Coffee

## Goal

More concentrated cup at 190 g water — same dose and water charge as [V60 Switch](../v60-switch/immersion-sweetness-body.md) for method comparison.

Original title: *24 kliki, 190 g* — pour-over variant at 1:12.7 ratio.

## Equipment

- Brewer: V60
- Filter: not documented
- Grinder: Comandante
- Kettle: not documented
- Scale: not documented

## Parameters

- Coffee dose: 15 g
- Water amount: 190 g
- Brew ratio: 1:12.7 (15 g : 190 g)
- Water temperature: 92°C
- Grind size: Comandante 24 clicks
- Water type: not documented

## Pour Structure

| Stage | Water added | Total water | Start time | End time | Notes |
|---|---:|---:|---|---|---|
| Bloom | 30 g | 30 g | 0:00 | not documented | bloom time not in source |
| Pour #1 | 45 g | 75 g | not documented | not documented | |
| Pour #2 | 40 g | 115 g | not documented | not documented | |
| Pour #3 | 40 g | 155 g | not documented | not documented | |
| Pour #4 | 35 g | 190 g | not documented | not documented | |
| Pour #5 | — | — | — | — | not applicable |

## Pouring Technique

- Pour style: not documented
- Center pour / circular pour: not documented
- Agitation: not documented
- Swirl: not documented
- Stirring: not documented
- Flow rate: not documented
- Bed shape target: not documented

## Timing

- Bloom time: not documented
- Drawdown time: not documented
- Total brew time: ~3:30

## Result

- Sweetness: not documented
- Body: not documented
- Acidity: not documented
- Bitterness: not documented
- Clarity: not documented
- Strength: higher — more concentrated
- Aftertaste: not documented
- Taste after cooling: not documented

## Serving And Tasting

- Recommended resting time after brewing: not documented
- First tasting point: not documented
- Optimal tasting temperature: not documented
- Flavor evolution as coffee cools: not documented

## Extraction Logic

- **190 g water:** lower total water than 200 g recipes → higher concentration (documented Result).
- **24 clicks:** same grind reference as [Balanced](./balanced.md); Switch uses 24–25 at 190 g with documented sweetness/body on immersion.
- **Four pours after bloom:** 45 + 40 + 40 + 35 g to 190 g — similar pour shape to [Stable](./stable.md) but scaled to 190 g.

## Tuning Guide

| Goal | Change |
|---|---|
| More sweetness | [V60 Switch](../v60-switch/immersion-sweetness-body.md) — documented high sweetness at 190 g |
| More body | [Stable](./stable.md) at 200 g; Switch immersion |
| More strength | already more concentrated — finer than 24 — not tested |
| Less bitterness | not documented |
| Less acidity | not documented |
| Less watery taste | not documented |
| Better taste after cooling | [Stable](./stable.md) |
| Less concentration | [Balanced](./balanced.md) at 200 g |

## Warning Signs

| Symptom | Likely cause | Adjustment |
|---|---|---|
| Watery / thin | underextraction — too coarse | finer; full pours to 190 g |
| Harsh bitterness | overextraction | coarser; lower temperature |
| Sourness / acidity | underextraction | finer |
| Slow drawdown | too fine | coarser |
| Fast drawdown | too coarse | finer |
| Too concentrated | expected at 190 g | [Balanced](./balanced.md) at 200 g |

## Historical Notes

- Original source: `## V60 #4 — 24 kliki, 190 g`.
- Former filenames: `24-kliki-190-g.md`, `woda-jak-switch.md`.
- **190 g** enables A/B with [V60 Switch](../v60-switch/immersion-sweetness-body.md) at 15 g / 190 g.
- Intended comparison with Switch after workflow is logged on both methods.

## Evidence

| Item | Value | Source | Confidence |
|---|---|---|---|
| Parameters, pour table, timing, Result | As documented | Robusta Honey source of truth (2026-06-03) | **High** |
| Bloom time, sensory detail beyond concentration | not documented | No source | — |

**Overall confidence:** **High** (parameters and workflow); **Medium** for full sensory Result

## Related Recipes

- See also:
  - [V60 Switch — Immersion, sweetness + body](../v60-switch/immersion-sweetness-body.md)
  - [V60 — Balanced](./balanced.md)
  - [V60 — Stable](./stable.md)
  - [V60 — Strong](./strong.md)

## Future Experiments

- Log bloom time; full Result vs Switch at same 15 g / 190 g.
- Taste after cooling.
